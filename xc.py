...
def _road_setup_section(m, setup, races, counts, _json, rename_js):
    ...
    if setup:
        ...
        event_card = (
            ...
            '<label style="margin-top:.4rem">Venmo username (optional)</label>'
            f'<input name="venmo" value="{escape(s.get("venmo") or "")}" '
            f'placeholder="@YourEvent (or paste your venmo.com link)" style="max-width:300px">'
            '<div class="muted" style="font-size:.8rem;margin:.2rem 0">With a fee <b>and</b> a Venmo set, '
            'runners are sent to Venmo to pay their total right after registering. No Venmo → the fee is '
            'just shown as owed (collect at packet pickup).</div>'
